...
def test_t25b_multi_chunk_conflict_in_second_chunk():
    ...
    chunk1 = _make_chunk("s1", "The rate is 300%.", chunk_id="c1")
    chunk2 = _make_chunk("s2", "The fee is 30%.",   chunk_id="c2")
    result = run(ctx, [chunk1, chunk2])
    assert result.has_conflict is True
    assert result.conflict_citation is not None
...
def test_t25c_stops_at_first_conflict():
    ...
    assert result.conflict_citation is not None
    assert "30" in result.conflict_citation.excerpt
    assert "5" not in result.conflict_citation.excerpt
    assert "1" not in result.conflict_citation.excerpt or "30" in result.conflict_citation.excerpt
